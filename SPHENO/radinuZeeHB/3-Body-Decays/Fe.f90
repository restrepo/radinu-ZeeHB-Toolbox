...
gFeFeFvFvi = 0._dp 
Call FeToFeFvFv(i_run,MFe,MFv,MVZ,MVWp,MHm,cplcFeFeVZL,cplcFeFeVZR,cplcFeFvcHmL,      & 
& cplcFeFvcHmR,cplcFeFvVWpL,cplcFeFvVWpR,cplFvFecVWpL,cplFvFecVWpR,cplFvFeHmL,           & 
...
& NVSss,NVSst,NVVss,NVVst,gTVZtemp,gTVWptemp,gTHmtemp,deltaM,epsI,check,gFeFeFvFvi)

gFeFeFvFv(i_run,:,:,:) = gFeFeFvFvi 
gT(i_run) = gT(i_run) + Sum(gFeFeFvFvi) 
 
gFeFvcFuFdi = 0._dp 
Call FeToFvcFuFd(i_run,MFv,MFu,MFd,MVWp,MHm,MFe,cplcFeFvcHmL,cplcFeFvcHmR,            & 
...
& NVSss,NVVss,gTVWptemp,gTHmtemp,deltaM,epsI,check,gFeFvcFuFdi)
...
Subroutine FeToFeFvFv(iIN,MFe,MFv,MVZ,MVWp,MHm,cplcFeFeVZL,cplcFeFeVZR,               & 
& cplcFeFvcHmL,cplcFeFvcHmR,cplcFeFvVWpL,cplcFeFvVWpR,cplFvFecVWpL,cplFvFecVWpR,         & 
& cplFvFeHmL,cplFvFeHmR,cplFvFvVZL,cplFvFvVZR,IntegralSs,IntegralSSss,IntegralSSst,      & 
& IntegralVs,IntegralVSss,IntegralVSst,IntegralVVss,IntegralVVst,NSs,NSSss,              & 
& NSSst,NVs,NVSss,NVSst,NVVss,NVVst,gTVZ,gTVWp,gTHm,deltaM,epsI,check,g,WriteContributions)

Implicit None 
 
Real(dp),Intent(in) :: MFe(3),MFv(3),MVZ,MVWp,MHm(3)

Complex(dp),Intent(in) :: cplcFeFeVZL(3,3),cplcFeFeVZR(3,3),cplcFeFvcHmL(3,3,3),cplcFeFvcHmR(3,3,3),            & 
& cplcFeFvVWpL(3,3),cplcFeFvVWpR(3,3),cplFvFecVWpL(3,3),cplFvFecVWpR(3,3),               & 
...
Real(dp),Intent(inout) :: gTVZ,gTVWp,gTHm(3)
...
coup(2) = Conjg(cplcFeFvVWpL(iIN,gt2)) 
coup(1) = Conjg(cplcFeFvVWpR(iIN,gt2)) 
coup(4) = Conjg(cplFvFecVWpL(gt3,gt1)) 
coup(3) = Conjg(cplFvFecVWpR(gt3,gt1))
Call IntegrateGaugeSS(Boson2,mass,coup,deltaM,epsI,IntegralVs,NVs,resR, check) 
If (gt3.Eq.gt2) Then 
resR=resR/2._dp 
End If
...
coup(4) = Conjg(cplFvFecVWpL(gt2,gt1)) 
coup(3) = Conjg(cplFvFecVWpR(gt2,gt1))
Call IntegrateGaugeSS(Boson2,mass,coup,deltaM,epsI,IntegralVs,NVs,resR, check) 
If (gt3.Eq.gt2) Then 
resR=resR/2._dp 
End If
...
coup2(4) = Conjg(cplcFeFvVWpL(iIN,gt2)) 
coup2(3) = Conjg(cplcFeFvVWpR(iIN,gt2))  
coup2(5) = cplFvFecVWpL(gt2,gt1) 
coup2(6) = cplFvFecVWpR(gt2,gt1) 
coup2(8) = Conjg(cplFvFecVWpL(gt3,gt1)) 
coup2(7) = Conjg(cplFvFecVWpR(gt3,gt1)) 
Call IntegrateGaugeST(Boson4, mass, coup2, deltaM, epsI,IntegralVVst,NVVst, resC, check) 
If (resC.ne.resC) resC = 0._dp
resC = -2._dp*resC 
resC= 1*resC ! color factor 
...
If (resD.ne.resD) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fe->Fe Fv Fv Propagator: VWp" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp
...
coup2(5) = cplFvFvVZL(gt2,gt3) 
coup2(6) = cplFvFvVZR(gt2,gt3) 
coup2(8) = Conjg(cplFvFecVWpL(gt2,gt1)) 
coup2(7) = Conjg(cplFvFecVWpR(gt2,gt1)) 
Call IntegrateGaugeST(Boson4, mass, coup2, deltaM, epsI,IntegralVVst,NVVst, resC, check) 
If (resC.ne.resC) resC = 0._dp
resC = -2._dp*resC 
resC= 1*resC ! color factor 
...
If (resS.ne.resS) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fe->Fe Fv Fv Propagator: VZ,VWp" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp  
...
coup2(1) = cplcFeFvVWpL(iIN,gt2) 
coup2(2) = cplcFeFvVWpR(iIN,gt2) 
coup2(3) = Conjg(cplcFeFvcHmL(iIN,gt2,i2)) 
coup2(4) = Conjg(cplcFeFvcHmR(iIN,gt2,i2))  
coup2(5) = cplFvFecVWpL(gt3,gt1) 
coup2(6) = cplFvFecVWpR(gt3,gt1) 
coup2(7) = Conjg(cplFvFeHmL(gt3,gt1,i2)) 
coup2(8) = Conjg(cplFvFeHmR(gt3,gt1,i2)) 
Call IntegrateGaugeSscalarS(Boson4, mass, coup2, deltaM, epsI,IntegralVSss,NVSss, resC, check) 
If (resC.ne.resC) resC = 0._dp
...
If (resS.ne.resS) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fe->Fe Fv Fv Propagator: VWp,conj[Hm]" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp  
...
Subroutine FeToFvcFuFd(iIN,MFv,MFu,MFd,MVWp,MHm,MFe,cplcFeFvcHmL,cplcFeFvcHmR,        & 
& cplcFeFvVWpL,cplcFeFvVWpR,cplcFuFdcVWpL,cplcFuFdcVWpR,cplcFuFdHmL,cplcFuFdHmR,         & 
& IntegralSs,IntegralSSss,IntegralVs,IntegralVSss,IntegralVVss,NSs,NSSss,NVs,            & 
...
Real(dp),Intent(in) :: MFv(3),MFu(3),MFd(3),MVWp,MHm(3),MFe(3)

Complex(dp),Intent(in) :: cplcFeFvcHmL(3,3,3),cplcFeFvcHmR(3,3,3),cplcFeFvVWpL(3,3),cplcFeFvVWpR(3,3),          & 
...
If (resS.ne.resS) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fe->Fv cFu Fd Propagator: VWp,conj[Hm]" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp  
...
End Module Fe3Decays_radinuZeeHB 
